  ...
  let pid = passages.reduce((result, passage) => {
	...
	for (const other of others) {
		...
		if (passageData.length === 0) {
			throw new Error(`${other.name} doesn't seem to contain any <tw-passagedata> elements. Make sure it's a Twine story.`);
		}

    for (const passage of passageData) {
      passage.setAttribute('pid', (++pid).toString());
      result = result.replace('</tw-storydata>', passage.outerHTML + '</tw-storydata>');
    }
	}
